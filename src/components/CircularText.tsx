...
const CircularText = ({ text, spinDuration = 20, onHover = 'speedUp', className = '' }: Props) => {
  ...
  const handleHoverStart = () => {
    ...
    switch (onHover) {
      ...
      case 'pause':
        controls.start({ rotate: start, scale: 1, transition: { type: 'spring', damping: 20, stiffness: 300 } });
        return;
  ...
  return (
    ...
    >
      {letters.map((letter, i) => {
        ...
        return (
          <span key={i} style={{ transform, WebkitTransform: transform }}>{letter}</span>
        );
      })}
    </motion.div>
  );
